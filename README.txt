...
On the root folder of the project you will find 4 scripts: compile.sh, peer.sh, test.sh and cleanup.sh
...
Start the Server
To start the server, run the server.sh script inside the build/ directory. Usage is as follows:
./server.sh <port>

Start a Peer
To start a peer, run the peer.sh script inside the build/ directory. Usage is as follows:
./peer.sh <svc_access_point> <port> <server_addr> <server_port>

Start the Test App
To start the Test App, run the test.sh script inside the build/ directory. Usage is as follows:
